...
public partial class DisplayManager {
    ...
    public bool Write(char character) {
        ...
        if (_escapedMode) {
            ...
            if (chr.Graphic) {
                ...
                _holdGraphicsCharacter = Display.Spc;
            }
        }
    ...
    public void SetCursorPosition(int index) {
        ...
        if (index < Display.Cols) {
            _cursor.Row = 0;
            _cursor.Col = index;
        }
        else {
            _cursor.Row = index / Display.Cols;
            _cursor.Col = index % Display.Cols;
        }
    ...
    private void ProcessDoubleHeight(ref Char chr) {
        ...
        if (_cursor.Row >= Display.Rows - 1) {
            return;
        }

        // get the character below
        // TODO refactor this with chr.GetCharBelow()
        var chrBelow = _display.Chars[(_cursor.Row + 1) * Display.Cols + _cursor.Col];
    ...
    private static Display CreateDisplay() {
        ...
        for (var i = 0; i < (Display.Rows + 1) * Display.Cols; i++) {
            var chr = new Char(Display.Spc)
            {
                Index = i
            };
