...
public static class Extensions {
    ...
    public static void Clear(this Display display) {

        // display is 25 rows (0-24) but we don't clear the last (status) row
        for (var i = 0; i < Display.Rows * Display.Cols; i++) {
        ...
        for (var i = 0; i < Display.Rows-1; i++) {
            display.RowReferences[i] = 0;
        }
    }

    public static void Flash(this Display display) {
    ...
    public static void Conceal(this Display display)
    {
        for (var i = 0; i < Display.Rows * Display.Cols; i++)
        {
            var c = display.Chars[i];
    ...
    public static void Reveal(this Display display)
    {
        for (var i = 0; i < Display.Rows * Display.Cols; i++)
        {
            var c = display.Chars[i];
    ...
    public static void ToggleConceal(this Display display) {
        for (var i = 0; i < Display.Rows * Display.Cols; i++) {
            var c = display.Chars[i];
            if (c.Concealed) {
    ...
    /// <returns></returns>
    public static bool IsRowReadOnly(this Display display, int row) {
    ...
    public static bool RowHasDoubleHeight(this Display display, int row) {
    ...
    public static Char GetChar(this Display display, int row, int col) {

        var isRowOutsideDisplayBounds = row is < 0 or >= Display.Rows;
        var isColumnOutsideDisplayBounds = col is < 0 or >= Display.Cols;
        ...
        return display.Chars[(row * Display.Cols) + col];
    }
    /// <summary>
    ...
    /// <param name="chr"></param>
    /// <returns></returns>
    public static Char GetCharBelow(this Display display, Char chr) {
        var index = chr.Index + Display.Cols;
        if (index < Display.Rows * Display.Cols) {
            return display.Chars[chr.Index + Display.Cols];
        }
    ...
    /// </summary>
    /// <returns></returns>
    public static List<Char> GetRemainderOfRow(this Display display, int row, int col) {

        var result = new List<Char>();

        for (var i = col + 1; i < Display.Cols; i++) {
